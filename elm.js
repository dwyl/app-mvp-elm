...
{
	if (region.Y.H === region.ak.H)
	{
		return 'on line ' + region.Y.H;
	}
	return 'on lines ' + region.Y.H + ' through ' + region.ak.H;
}
...
var _Http_toTask = F3(function(router, toTask, request)
{
	return _Scheduler_binding(function(callback)
	{
		...
		try {
			xhr.open(request.bi, request.ab, true);
		} catch (e) {
...
var $elm$url$Url$Url = F6(
	function (protocol, host, port_, path, query, fragment) {
		return {am: fragment, ar: host, ay: path, aA: port_, aD: protocol, aE: query};
	});
...
var $author$project$Pages$Auth$Model = F2(
	function (session, urls) {
		return {aM: session, ac: urls};
	});
var $author$project$Pages$Auth$GotAuthUrls = $elm$core$Basics$identity;
var $elm$json$Json$Decode$list = _Json_decodeList;
var $author$project$Pages$Auth$Url = F2(
	function (url, typeUrl) {
		return {aU: typeUrl, ab: url};
	});
...
var $elm$http$Http$request = function (r) {
	return $elm$http$Http$command(
		$elm$http$Http$Request(
			{a$: false, a1: r.a1, ba: r.ba, ap: r.ap, bi: r.bi, bG: r.bG, aT: r.aT, ab: r.ab}));
};
var $elm$http$Http$get = function (r) {
	return $elm$http$Http$request(
		{a1: $elm$http$Http$emptyBody, ba: r.ba, ap: _List_Nil, bi: 'GET', bG: $elm$core$Maybe$Nothing, aT: $elm$core$Maybe$Nothing, ab: r.ab});
};
...
var $author$project$Main$loadRoute = F2(
	function (maybeRoute, model) {
		var session = $author$project$Main$toSession(model.q);
		if (maybeRoute.$ === 1) {
			return _Utils_Tuple2(
				_Utils_update(
					model,
					{
						q: $author$project$Main$NotFound(session)
					}),
				$elm$core$Platform$Cmd$none);
		} else {
			if (!maybeRoute.a.$) {
				...
				return _Utils_Tuple2(
					_Utils_update(
						model,
						{
							q: $author$project$Main$Home(subModel)
						}),
				...
				if (maybeRoute.a.a.$ === 1) {
					...
					return _Utils_Tuple2(
						_Utils_update(
							model,
							{
								q: $author$project$Main$Auth(subModel)
							}),
					...
					return _Utils_Tuple2(
						_Utils_update(
							model,
							{
								q: $author$project$Main$Session(subModel)
							}),
	...
	return A2(
		...
		_List_fromArray(
			[
				...
				_List_fromArray(
					[
						$elm$html$Html$Attributes$href(url.ab)
					]),
				_List_fromArray(
					[
						A2(
			...
			[
				...
				if (!model.$) {
					...
					return A2(
						...
						_List_fromArray(
							[
								$elm$html$Html$text('logged in with token: ' + person.a8)
							]));
				}
			}()
			]),
